...
| 層 | 定義 |
|---|---|
| **AUI（Avatar UI）** | 物理生命と情報生命の相互干渉条件を設計・制御するインターフェース（新世代UI概念） |
...
AUI（Avatar UI）とは、物理生命と情報生命が重なって存在する空間において、相互に干渉し合う条件を設計・制御するインターフェースである。AUIはメタOS — 下位OS（Windows/macOS/Linux）の機能を使って、より上位の「目的」を管理する基盤。
...
物理生命と情報生命の共生を、概念ではなく実際に動く体験として成立させる。
