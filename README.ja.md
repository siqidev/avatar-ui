...
- **アイドルアニメーション** — たまごっち風ランダムコマ送り＋瞬き（アバター画像カスタマイズ可能）
- **共振モード** — 場レベルの注意ゲート: アバターが世界の出来事に気づき自律的に応答する
...
`src/renderer/public/` にPNG画像を配置するとアイドルアニメーションが有効になります:
...
有効時（AUIメニュー > 共振）、アバターは世界の出来事（例: Robloxでプレイヤーが近づいた）に気づき、人間の明示的な入力なしに応答します。無効時は直接メッセージにのみ応答します。
