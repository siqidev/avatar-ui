...
> **Tip:** アイデンティティファイルを別ディレクトリで管理したい場合、`.env`に`AVATAR_DIR=my-avatar`を設定すると、そのディレクトリ内の`BEING.md`、`pulse/`、`commands/` を読み込みます。`commands/` 配下のファイルは起動時にカスタム Discord スラッシュコマンドとしてロードされます（[オプション: Discord連携](#オプション-discord連携) 参照）。
...
| `ROBLOX_OWNER_USER_ID` | オーナーのRoblox UserID（数値。オーナー判定用。未設定時は全員external） |
...
| `X_OWNER_USER_ID` | オーナーのXユーザーID（数値。オーナー判定用。未設定時は全員external） |
...
`DISCORD_BOT_TOKEN` と `DISCORD_CHANNEL_ID` の両方を設定すると有効化されます。

| 変数 | 説明 |
|------|------|
| `DISCORD_BOT_TOKEN` | Discordボットトークン |
| `DISCORD_CHANNEL_ID` | 接続先チャンネルID |
| `DISCORD_OWNER_ID` | オーナーのDiscordユーザーID（数値。オーナー判定用。未設定時は全員external） |

Botに@メンションすると会話できます。Console側の会話もDiscordに転送されます。ツール承認はボタン操作（オーナーのみ）。Discord Developer Portalで`Guilds`・`GuildMessages`・`MessageContent` Intentを有効にしてください。

#### カスタムスラッシュコマンド（オーナー専用）

`$AVATAR_DIR/commands/` に `.ts` / `.mjs` / `.js` ファイルを置き、`AvatarCommand` をエクスポートします（[`src/discord/avatar-command.ts`](src/discord/avatar-command.ts) 参照）。起動時に動的importされ、接続中のguildにスラッシュコマンドとして登録されます。ローダーがオーナー判定（`ownerOnly: true`）と ChatInput / StringSelectMenu / ModalSubmit のルーティング（`customIdPrefix` 最長一致）を担います。
...
| **InputGate（ロールベース）** | チャネルごとにオーナー判定 — 外部ユーザーは同一媒体の応答ツールのみ使用可（ハードコード、.envで緩和不可） |
