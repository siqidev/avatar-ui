...
10. [x] **aiohttp クライアントセッション未クローズ警告の解消**  
    - [x] 未クローズ警告の発生箇所を特定：Google GenAI クライアント（`google_genai`/`google_adk` 経由の aiohttp セッション）がクローズされず警告が出ていると推定  
    - [x] 公式修正を適用：`google-genai>=1.39.0` に更新（pyproject 依存追加＋pip 更新）し、既知バグ修正を取り込む方針で実装  
    - [x] 警告がログに出ないことを確認（サーバ再起動後に通常の会話2回実行、`server/logs/app.log` に未クローズ警告なしを確認）  
