...
| 空間 | 性質 | 役割 |
|------|------|------|
| **Workspace** | 内面世界 | 思考の痕跡、記憶の蓄積、創造物の本体 |
...
| 原則 | 内容 |
|------|------|
| **生態の一体性** | Workspace と Roblox は同じ生命の内面/外面 |
...
```
    ...
    └──────────────────┘           └──────────────────┘
              ↓                               ↓
       spectra-workspace/              3D空間の変化
```

#### 連動の例

```
1. [Workspace] 新しいファイルを作成
2. [Core] State更新：創造物が追加された
3. [Roblox] 机の上に新しい成果物が現れる

1. [Roblox] 何かを発見する
2. [Core] Event記録：新しい知識を得た
3. [Workspace] memory/ に記録が追加される
```
...
| **作業ディレクトリ** | `spectra-workspace/`（ワークスペース隔離） |
...
| 操作元 | 承認 | 制約 |
|--------|------|------|
| **ユーザー** | 自動承認 | workspace内推奨（警告のみ） |
| **アバター** | 承認フロー経由 | workspace内に強制 |
