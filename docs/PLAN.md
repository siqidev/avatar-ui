...
概念設計はPROJECT.md（媒体横断契約表・4モード生起条件）で確定済み。

**確定済み（L1〜L3）:**
- X API Pay-Per-Use（従量制）。課金はPost read単位（~$0.005/件）。個人用途なら微小
- Console + Roblox + X 併存（既存のenqueueに合流。排他制御不要、優先度はFIFO）
- 入力: Webhook（Account Activity API）で即時取得。CRC検証+署名検証。Roblox観測と同じイベント駆動パターン
- 出力: 自発ポスト（x_post）は事前承認不要で即日運用可。返信（x_reply）はX事前承認が必要（Developer Portal経由申請、数週間〜数ヶ月）
- 280文字制限はツール定義でバリデーション（分割不要）
- いいね等: メンション取得時にpublic_metricsを付加（追加コストゼロ。非Postイベントは課金対象外）。環境顕現 = 共振材料
- 承認制御: 既存TOOL_AUTO_APPROVEに乗せる。別ロジック不要
- 表示: Xペイン新設（中央列3段化: Canvas / X / Roblox）
- 自律行動: Pulse複数化（main session一本、PULSE_OK非保存）。設定は.env（PULSE_GENERAL_CRON + PULSE_X_CRON）
- 文脈: 単一AIセッション（媒体識別で応答先を区別。文脈は同一個体として共有）
- 有効化: .envにXトークンがあればX機能有効化（Robloxの`ROBLOX_SERVER_URL`と同パターン）
- セキュリティ: 外部メンション=プロンプトインジェクションリスク
- 規約遵守: Automatedラベル有効化 + bio開示 + opt-out即時対応。自動いいね禁止

**v0.4.0段階的リリース:**
- Phase 1: x_post（自発ポスト）+ Webhook受信（Xペイン表示）— 承認不要で即日可
- Phase 2: x_reply（返信）— X事前承認取得後に有効化

**未決定（L4: 実装詳細）:**
- ツール名（仮称: x_reply, x_post）
- 権限制御の方式（仮称: InputGate）
- 出力インターフェースの具体設計（ChannelProjection拡張の形）
- OAuthトークン管理の具体実装（変数名・リフレッシュ等）
