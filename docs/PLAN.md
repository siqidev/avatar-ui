...
- ~~PULSE_CRONデフォルトを1日1回に変更~~ → pulse/ディレクトリシステムに移行（v0.5.1）
...
- x_replyツール（メンション返信）
- x_quote_repostツール（引用リポスト、280文字制限）
...
**問題1（2026-03-18解決）: App permissionsにDM権限がなかった**
- App permissionsが「読み書き」のみ → Webhook登録・CRC・購読は成功するがイベント配信されない
- 「読み書きおよびダイレクトメッセージ」に変更 + アクセストークン再生成 + 再購読で解決
- 教訓: Account Activity APIはDM権限必須。権限変更後は既存トークンに反映されないため再生成が必要

**問題2（2026-04-10解決）: Webhook無効化**
- XはWebhookに定期CRC検証を送る（約24時間ごと）。VPS再起動等で応答できないとWebhookが無効化され、以降イベントもCRCも来なくなる（自動復旧しない）
- console.x.comのWebhook管理ページから「再検証」で復旧
- 教訓: プロセス再起動後はWebhookの有効性を確認する必要がある
...
- **完全リファクタリング** — 計画正本は docs/refactoring-plan.md（2026-06-12 策定。Phase 0〜8 = リファクタ本体、Phase 9 = γ軸でα/β議論決着ゲート付き。下記γ軸リファクタ節と整合）
...
- ~~**Avatar Space構造設計（refs/方式）**~~ — 実装済み。avatar-space直下はrw（アバターの自由活動領域）、`refs/`ディレクトリ配下はro（参照専用）。アプリ層（filesystem-service.ts）で`refs/`への書き込みを拒否。OS側のbind mount/権限に依存しない
  ...
  - ~~**XPulse素材アクセス**~~: 実装済み → pulse/ディレクトリシステムに統合（v0.5.1）。各パルスファイルのtools設定でfs_read/fs_listを開放
...
| 4 | 往復回路（ReciprocityLoop） | 実装（enqueue直列化+chain使い捨て） | 中断処理→優先度→再解釈 |
