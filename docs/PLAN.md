...
- **X チャネル統合** — 概念設計はPROJECT.md（媒体横断契約表・4モード生起条件）で確定済み。以下は実装プロファイル:
  ...
  **入力経路:**
  - メンションポーリング（GET /2/users/:id/mentions, 300req/15min）
  - 将来: Account Activity API Webhook（3サブスクリプション制限）

  **出力ツール:**
  - `x_reply` — メンションへの返信（280文字制限、分割ロジック必要）
  - `x_post` — 自発ポスト（Pulse経由の定期発信等）
  - POST /2/tweets: 100ポスト/15min (Per User), 月間キャップはティア依存

  **チャネル抽象化の実装順序:**
  1. InputGate — field-runtime手前でチャネル別ツール権限を制限（Roblox観測からx_reply不可等）
  2. ChannelProjection interface拡張 — 出力側の媒体抽象化（Console/X/Roblox共通インターフェース）
  3. X adapter実装 — ポーリング + 投稿 + OAuth管理

  **セキュリティ:**
  - 外部ユーザーからのメンション = プロンプトインジェクションリスク（Robloxと同等以上）
  - 公開API操作 = レート制限・アカウント凍結リスク
