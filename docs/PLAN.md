...
**確定済み（L1-L2: 要件 + L3: 方式選択）:**
- X API Pay-Per-Use（従量制）でメンション読取可能。固定月額不要
- Console + Roblox + X 併存（既存のenqueueに合流。排他制御不要、優先度はFIFO）
- 入力: Webhook（Account Activity API）で即時取得。Roblox観測と同じイベント駆動パターン
- 出力: 返信・自発ポスト。280文字制限はツール定義でバリデーション（分割不要）
- いいね等: メンション取得時にpublic_metricsを付加（追加コストゼロ）。環境顕現 = 共振材料
- 承認制御: 既存TOOL_AUTO_APPROVEに乗せる。別ロジック不要
- 表示: Xペイン新設（投稿内容・メンション・エンゲージメントを表示）
- 自律行動: Pulse複数化（main session一本、PULSE_OK非保存）。PULSE-x.mdで判断基準を定義
...
- Xペインのレイアウト（7ペイン化 or 既存ペインの再構成）
- Pulse複数化の設定形式（.env / settings.json / ディレクトリ規約）
