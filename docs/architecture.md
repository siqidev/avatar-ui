...
src/
  ...
  main/
    ...
    settings-store.ts         設定ストア（テーマ・モデル・言語永続化 → data/settings.json）
    menu.ts                   カスタムメニュー（テーマ・モデル・言語radio + About）
...
AIへの転送は異常対応に必要な信号のみに限定する（フィードバックループ防止）。Renderer表示も同じポリシーを適用し、通常モード（DEV_MODE=off）では重要イベントのみ表示する。DEV_MODE=onで全イベント表示。

| イベント | 条件 | AI転送 | Renderer表示（通常） | Renderer表示（devMode） |
|---------|------|--------|---------------------|----------------------|
| `player_chat` / `player_proximity` | 常時 | する | する | する |
| `command_ack` | `success===true` | しない | しない | する |
| `command_ack` | 失敗 | する | する | する |
| `npc_follow_event` | `started` / `stopped` | しない | しない | する |
| `npc_follow_event` | `lost` / `path_failed` | する | する | する |
| `projection_ack` | 成功 | しない | しない | する |
| `projection_ack` | 失敗 | する | する | する |
| `roblox_log` | 常時 | しない | しない | する |
...
テーマ・モデル・言語の3設定はメニューからランタイムで変更可能。`data/settings.json`に永続化。

| 設定 | デフォルト | 変更手段 | 反映先 |
|------|----------|---------|--------|
| theme | `"modern"` | AUIメニュー > Theme radio | Renderer（CSS変数 + xterm） |
| model | GROK_MODEL環境変数の値 | AUIメニュー > Model radio | chat-session-service.ts（次のAPI呼出から反映） |
| locale | `"ja"` | AUIメニュー > Language radio | 全テキスト（i18n辞書。変更時Rendererリロード） |

起動時: `loadSettings(dataDir, config.model)` でsettings.jsonを読み込み。ファイルが存在しない/破損時は環境変数のデフォルト値を使用。
...
| ランタイム設定（テーマ・モデル・言語） | data/settings.json + settings-store.ts |
