...
| `RESONANCE_MODE` | — | `"off"` | 共振モード初期値（on: 観測→AI転送+応答生成）。ランタイムはsettings-storeが正本 |
...
| resonance | RESONANCE_MODE環境変数の値 | AUIメニュー > Resonance checkbox | field-runtime.ts（観測→AI転送ゲート。即時反映） |

起動時: `loadSettings(dataDir, config.model, config.resonanceMode)` でsettings.jsonを読み込み。ファイルが存在しない/破損時は環境変数のデフォルト値を使用。
