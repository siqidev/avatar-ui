...
- **Main**: FieldRuntime（6要素）、Pulse(cron)、観測受信(HTTP)、X Webhook受信(HTTP)、Grok API呼出、永続化
...
- x_reply: メンションへの返信（Phase 2、X事前承認取得後にX_REPLY_APPROVED=onで有効化）
...
| `X_REPLY_APPROVED` | — | `"off"` | X返信機能（Phase 2: X事前承認後にonに設定） |
...
- `isXReplyEnabled()`: isXEnabled() + X_REPLY_APPROVED=on の場合（Phase 2）
