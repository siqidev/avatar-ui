...
- **セッションイベントバス**: runtime/session-event-bus.ts — FieldRuntimeが`stream.item`/`monitor.item`イベントをpublish。ipc-handlersがsubscribeしChannelProjection経由でRendererに投影。トランスポート非依存（将来WebSocket購読者も接続可能）
