...
- **Main**: FieldRuntime（6要素）、Pulse(cron)、XPulse(cron)、観測受信(HTTP)、X Webhook受信(HTTP)、Grok API呼出、永続化
...
Pulseと同じnode-cron + プロンプトファイルのパターンで、X投稿に特化した定期発火。`XPULSE_CRON`間隔（デフォルト: `0 5,9 * * *` = JST 14:00/18:00）で発火し、XPULSE.mdの内容に従ってx_postツールでXに投稿する。

- **XPULSE.md注入**: Pulseと同様、`loadXpulse()`で読み込み動的入力として送信。`forceSystemPrompt=true`
- **XPULSE_OKプロトコル**: `XPULSE_OK`接頭辞で応答した場合、投稿不要と判断しスキップ
- **X連携必須**: `isXEnabled(config)`がfalseの場合、XPulse自体を起動しない
- **source/channel**: `source="xpulse"`, `channel="x"`。PulseのStreamペイン表示とは区別される

#### メッセージキューと直列化（enqueue）

全入力（user/pulse/xpulse/observation）は`enqueue()`を経由してAI呼び出しを直列化する。Promise chainパターンで実装し、前のジョブ完了後に次のジョブを実行。JSシングルスレッド+イベントループにより、同時発火（Pulse発火中にユーザー入力、観測イベント到着等）でもキュー順に処理される。優先度制御はなくFIFO。凍結時はジョブをスキップ（onSkipコールバックで呼び出し元のPromiseをreject）
- **Renderer**: 7ペインの描画＋ユーザー入力の送信＋イベント購読のみ
- **セッションイベントバス**: runtime/session-event-bus.ts — FieldRuntimeが`stream.item`/`monitor.item`イベントをpublish。ipc-handlersがsubscribeしChannelProjection経由でRendererに投影。session-ws-server.tsもsubscribeしWebSocketクライアントに配信
- **セッションWebSocketサーバー**: runtime/session-ws-server.ts — event busイベントを外部クライアントにリアルタイム配信。接続時にsession.state（場の状態+履歴）を初回送信。stream.post受信にも対応。`SESSION_WS_TOKEN`設定時はtoken認証を行う（デフォルトport: 3002）
- **媒体投影**: channel-projection.ts（ChannelProjection）がRenderer送信＋メッセージ整形を集約。ipc-handlersはイベントバス購読→投影転送のみ。新チャネル追加時はChannelProjection実装を追加しipc-handlers無変更
...
| `onStreamReply(cb)` | stream.reply | AI応答（source: user/pulse/xpulse/observation） |
...
**Main → Renderer（ToRendererMessage）**: stream.reply / field.state / integrity.alert / observation.event / x.event
...
| Stream | stream | 場の全入出力の統合ストリーム（human↔AI対話 + Pulse + XPulse + 観測 + ツール可視化） | 読み書き |
...
source + channelの組み合わせでAIが使用可能なツールを制限する二重防御:

| source | channel | 許可ツール |
|--------|---------|-----------|
| user/pulse/xpulse | console/x | 全ツール |
| observation | roblox | roblox_action + 読み取り系（save_memory, fs_list, fs_read） |
| observation | x | x_reply + 読み取り系 |
| observation | console | 読み取り系のみ |
...
| input-gate.ts | source+channelベースのツール権限制御 |
...
| `XPULSE_CRON` | — | `"0 5,9 * * *"` | XPulse発火間隔（cron式、UTC。JST 14:00/18:00） |
