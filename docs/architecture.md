...
- **プロセス**: FieldRuntime（6要素）、Pulse(cron)、XPulse(cron)、観測受信(HTTP)、X Webhook受信(HTTP)、Grok API呼出、永続化、Discord窓口(discord.js)、Console HTTP配信、Session WSサーバー
...
node-cronで`PULSE_CRON`間隔（デフォルト: `0 6 * * *` = JST 15:00、1日1回）に自動発火。PULSE.mdの内容をユーザー入力としてAIに送信し、AIが状況判断して応答する。
...
| `PULSE_CRON` | — | `"0 6 * * *"` | Pulse発火間隔（cron式、UTC。JST 15:00、1日1回） |
