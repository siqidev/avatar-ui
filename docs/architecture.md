...
src/
  ...
  runtime/                    Electron非依存のランタイム基盤
    ...
    fs-request-handler.ts     FS dispatcher（IPC/WS共通の薄い分岐。SSOT）
  services/
  ...
  tools/
    ...
    x-reply-tool.ts           LLMツール: x_reply（X返信）
    x-quote-repost-tool.ts    LLMツール: x_quote_repost（X引用リポスト）
  x/
  ...
  discord/
    ...
    avatar-command.ts           カスタムslashコマンドのcontract定義
    avatar-command-loader.ts    $AVATAR_DIR/commands/動的ロード+ルーティング
  memory/
  ...
  shared/
    ...
    fs-rpc-schema.ts          FS RPC（WS）エンベロープ型定義（Zod。fs.importFileは公開subset外）
...
**テンプレート派生値**: テンプレート内で`_`プレフィックス付きプレースホルダを使うと、データから自動計算された値が展開される。
- `{_alt_bar}` / `{_dist_bar}` / `{_dur_bar}` — 数値バーグラフ（▰▰▱▱形式）
- `{_minimap}` — 緯度経度から5×7グリッドのミニマップ
- `{_coords}` — 座標テキスト（例: `46.1°N  64.8°W`）
- `{_date}` — タイムスタンプをUTC文字列に変換（例: `2026-04-07 05:24 UTC`）

**実行フロー**（cron発火時）:
1. **データ取得**: `source`設定時、URLからデータをフェッチ
2. **プログラム表示**: `target` + `template`設定時、テンプレート（派生値含む）でデータを整形しRoblox MessagingService経由で直接表示（AI不使用）。データ変更有無に関わらず毎回表示を送信（Robloxサーバー再起動時のSurfaceGui消失に対応）
3. **AI送信**: マークダウン本文（AI指示）がある場合、データ変更時のみ取得データを添付してAIに送信（MD5ハッシュで重複検出）

**起動時表示復元**: Robloxサーバー起動時に`GET /observation/display-state`でVPSから最新表示スナップショットをpullし即表示する。pulse-runnerがcron更新のたびに最新表示テキストをメモリに保持し、観測サーバー（port 3000）が同エンドポイントで返却する。
...
- **Xチャネル特化**: `channel: x`の場合、直近投稿履歴を自動注入（重複防止）、X投稿系ツール（x_post/x_reply/x_quote_repost）未使用の応答は抑制
...
- **セッションWebSocketサーバー**: runtime/session-ws-server.ts — event busイベントを外部クライアントにリアルタイム配信。接続時にsession.state（場の状態+履歴+pendingApprovals）を初回送信。stream.post受信・ツール承認応答にも対応。30秒間隔のping/pongで半開き接続を検出・切断。`SESSION_WS_TOKEN`設定時はtoken認証、`SESSION_WS_ALLOWED_ORIGINS`設定時はOrigin allowlist検証（CSWSH対策の多層防御）を行う（デフォルトport: 3002）
...
- **カスタムslashコマンド**: discord/avatar-command-loader.ts — `$AVATAR_DIR/commands/*.{mjs,js,ts}` を起動時に動的import（pathToFileURL経由）し、Discord接続先guildにスラッシュコマンドとして登録。`AvatarCommand` contract（discord/avatar-command.ts）に従う実装を要求。owner enforcement (`ownerOnly: true`) と `isEnabled(config)` ゲートはloader側で一元処理。InteractionCreate（ChatInput / StringSelectMenu / ModalSubmit）は最長一致するcustomIdPrefixで該当コマンドにルーティングする
...
| kind | 方向 | 概要 |
|------|------|------|
| `stream.item` | Server → Client | ストリームアイテム（human入力/AI応答。source: user/pulse/observation） |
...
Electron preloadの `window.fieldApi` は `/field-api-polyfill.js` でスタブ注入される（ポリフィル本体は `runtime/field-api-polyfill.ts` が生成）。ポリフィルはWEB_CAPABILITIESを宣言し、terminal等のDesktop限定能力は明示的にrejectする。
HTTPとWebSocketは同一ポート（SESSION_WS_PORT, デフォルト3002）で提供される。
token認証: `SESSION_WS_TOKEN` 設定時、`?token=xxx` でアクセス → Cookie自動設定。
Origin allowlist: `SESSION_WS_ALLOWED_ORIGINS` 設定時、WS upgradeを指定Originのみ許可（CSWSH対策の多層防御）。
Cache-Control: HTML/ポリフィルは常時再生成（no-cache）+ キャッシュバスター付きURL、アセット（js/css/画像）は弱ETag（size+mtime）とLast-Modifiedによる304 Not Modified応答でCloudflareトンネル帯域を節約。
devMode注入: DEV_MODEの値をポリフィル経由でブラウザ側に伝播。
CSP/theme書き換え: index.htmlのCSP metaタグとtheme-init.jsパスは正規表現で1件マッチを要求し、マッチ失敗時はfail-fast（Vite出力変更の早期検知）。

**FS RPC（ブラウザ→サーバー）**: ポリフィルの `fsList/fsRead/fsWrite/fsMutate/fsRootName` は同じWS接続上で `fs.request`/`fs.response` エンベロープ（`shared/fs-rpc-schema.ts`）を交換する。サーバー側は `runtime/fs-request-handler.ts` の `dispatchFsRequest` でElectron IPCと同一の実装に委譲する（SSOT）。`fs.importFile` はWS公開subsetから除外（任意絶対パスをサーバー入口に晒さないため）、ブラウザ側ではエラーを返す。
...
| `x_reply` | tools/x-reply-tool.ts | Xメンション返信 |
| `x_quote_repost` | tools/x-quote-repost-tool.ts | X引用リポスト（280文字以内） |
...
- 共振ゲート: メンション（干渉）は共振OFF時もAI転送する。環境顕現（将来追加時）のみ共振ゲート対象

### 出力経路（場→X）

- x_post: AIが自発的にポスト
- x_reply: メンションへの返信
- x_quote_repost: 引用リポスト（他者の投稿にコメントを添えて共有）
...
| ファイル | 責務 |
|---------|------|
| x-api-repository.ts | OAuth 1.0a署名 + X API v2呼出（createPost/createReply/createQuoteRepost） |
...
| x-reply-tool.ts | x_replyツール定義 |
| x-quote-repost-tool.ts | x_quote_repostツール定義（引用リポスト） |
...
| `AVATAR_DIR` 配下 | — | `${AVATAR_DIR}/pulse/` | パルスディレクトリ（各.mdファイルがcronタスク。`AVATAR_DIR`から自動導出。環境変数不要） |
...
| `SELF_ANALYSIS_JOURNAL_FILE` | — | — | 自己分析ジャーナル（カスタム`/log`プラグイン用）の追記先`.md`絶対パス |
| `SELF_ANALYSIS_REPO_DIR` | — | — | 自己分析ジャーナルの`git commit`実行ディレクトリ絶対パス |
...
| Pulse定義 | pulse/ディレクトリ（$AVATAR_DIR/pulse/。自動導出） |
