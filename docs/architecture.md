...
全観測イベントはRendererに表示するが、AIへの転送は異常対応に必要な信号のみに限定する（フィードバックループ防止）。

| イベント | 条件 | AI転送 |
|---------|------|--------|
| `player_chat` / `player_proximity` | 常時 | する（本来の観測入力） |
| `command_ack` | `success===true` | しない（ツールループで既知） |
| `command_ack` | 失敗 | する（リトライ判断） |
| `npc_follow_event` | `started` / `stopped` | しない（正常遷移） |
| `npc_follow_event` | `lost` / `path_failed` | する（エラー対応） |
| `projection_ack` | 成功 | しない |
| `projection_ack` | 失敗 | する |
| `roblox_log` | 常時 | しない（表示+ログのみ） |
